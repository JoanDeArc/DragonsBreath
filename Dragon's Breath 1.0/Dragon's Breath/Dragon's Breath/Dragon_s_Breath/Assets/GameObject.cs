...
namespace Dragon_s_Breath.Assets
{
    ...
    {
        protected Matrix modelOrientation;
        protected readonly int model_;

        public Matrix ModelOrientation { get { return modelOrientation; } }

        public GameObject(int model, Vector3 position)
        {
            model_ = model;
            modelOrientation = Matrix.Identity * Matrix.CreateTranslation(position);
        }
